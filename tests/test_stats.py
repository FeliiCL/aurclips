...
def _db(clips: list[tuple[float, str, bool, int]]) -> State:
    ...
    for i, (dur, title, marked, views) in enumerate(clips):
        start = i * 100.0
        clip = types.SimpleNamespace(start=start, end=start + dur, title=title,
                                     description="desc", tags=[], score=None,
                                     marked=marked)
        clip_id = db.add_clip(vid, i, clip)
        db.clip_rendered(clip_id, "x.mp4")
        db.clip_uploaded(clip_id, f"yt{i}", None)
        db.record_metrics([(clip_id, views, 0)])
    return db
...
def test_con_muestra_suficiente_sale_el_ganador_de_cada_dimension():
    lineas = review_header(_db(CORTO_Y_BUENO + LARGO_Y_FLOJO))
    texto = " ".join(lineas)
...
def test_la_cabecera_cabe_en_la_pantalla():
